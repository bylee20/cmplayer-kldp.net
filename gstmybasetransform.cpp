#include "gstmybasetransform.hpp"
...
#define GST_MY_BASE_TRANSFORM_GET_PRIVATE(obj)  \
    (G_TYPE_INSTANCE_GET_PRIVATE ((obj), GST_TYPE_MY_BASE_TRANSFORM, GstMyBaseTransformPrivate))

struct _GstMyBaseTransformPrivate
{
  /* QoS *//* with LOCK */
...
static void gst_my_base_transform_class_init (GstMyBaseTransformClass * klass);
static void gst_my_base_transform_init (GstMyBaseTransform * trans,
    GstMyBaseTransformClass * klass);
static GstFlowReturn gst_my_base_transform_prepare_output_buffer (GstMyBaseTransform
    * trans, GstBuffer * input, GstBuffer ** buf);

GType
gst_my_base_transform_get_type (void)
{
  ...
  if (g_once_init_enter (&base_transform_type)) {
    ...
    static const GTypeInfo base_transform_info = {
      sizeof (GstMyBaseTransformClass),
      NULL,
      NULL,
      (GClassInitFunc) gst_my_base_transform_class_init,
      NULL,
      NULL,
      sizeof (GstMyBaseTransform),
      0,
      (GInstanceInitFunc) gst_my_base_transform_init,
    };

    _type = g_type_register_static (GST_TYPE_ELEMENT,
	"GstMyBaseTransform", &base_transform_info, G_TYPE_FLAG_ABSTRACT);
    g_once_init_leave (&base_transform_type, _type);
  }
  return base_transform_type;
}

static void gst_my_base_transform_finalize (GObject * object);
static void gst_my_base_transform_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_my_base_transform_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static gboolean gst_my_base_transform_src_activate_pull (GstPad * pad,
    gboolean active);
static gboolean gst_my_base_transform_sink_activate_push (GstPad * pad,
    gboolean active);
static gboolean gst_my_base_transform_activate (GstMyBaseTransform * trans,
    gboolean active);
static gboolean gst_my_base_transform_get_unit_size (GstMyBaseTransform * trans,
    GstCaps * caps, guint * size);

static gboolean gst_my_base_transform_src_event (GstPad * pad, GstEvent * event);
static gboolean gst_my_base_transform_src_eventfunc (GstMyBaseTransform * trans,
    GstEvent * event);
static gboolean gst_my_base_transform_sink_event (GstPad * pad, GstEvent * event);
static gboolean gst_my_base_transform_sink_eventfunc (GstMyBaseTransform * trans,
    GstEvent * event);
static gboolean gst_my_base_transform_check_get_range (GstPad * pad);
static GstFlowReturn gst_my_base_transform_getrange (GstPad * pad, guint64 offset,
    guint length, GstBuffer ** buffer);
static GstFlowReturn gst_my_base_transform_chain (GstPad * pad,
    GstBuffer * buffer);
static GstCaps *gst_my_base_transform_getcaps (GstPad * pad);
static gboolean gst_my_base_transform_acceptcaps (GstPad * pad, GstCaps * caps);
static gboolean gst_my_base_transform_setcaps (GstPad * pad, GstCaps * caps);
static GstFlowReturn gst_my_base_transform_buffer_alloc (GstPad * pad,
    guint64 offset, guint size, GstCaps * caps, GstBuffer ** buf);

/* static guint gst_my_base_transform_signals[LAST_SIGNAL] = { 0 }; */

static void
gst_my_base_transform_finalize (GObject * object)
{
  GstMyBaseTransform *trans;

  trans = GST_MY_BASE_TRANSFORM (object);
...
static void
gst_my_base_transform_class_init (GstMyBaseTransformClass * klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);

  g_type_class_add_private (klass, sizeof (GstMyBaseTransformPrivate));

  parent_class = GST_ELEMENT_CLASS(g_type_class_peek_parent (klass));

  gobject_class->set_property = gst_my_base_transform_set_property;
  gobject_class->get_property = gst_my_base_transform_get_property;

  g_object_class_install_property (gobject_class, PROP_QOS,
      g_param_spec_boolean ("qos", "QoS", "Handle Quality-of-Service events",
	  DEFAULT_PROP_QOS, GParamFlags(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

  gobject_class->finalize = gst_my_base_transform_finalize;

  klass->passthrough_on_same_caps = FALSE;
  klass->event = GST_DEBUG_FUNCPTR (gst_my_base_transform_sink_eventfunc);
  klass->src_event = GST_DEBUG_FUNCPTR (gst_my_base_transform_src_eventfunc);
}

static void
gst_my_base_transform_init (GstMyBaseTransform * trans,
    GstMyBaseTransformClass * bclass)
{
  GstPadTemplate *pad_template;

  GST_DEBUG ("gst_my_base_transform_init");

  trans->priv = GST_MY_BASE_TRANSFORM_GET_PRIVATE (trans);
  ...
  gst_pad_set_getcaps_function (trans->sinkpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_getcaps));
  gst_pad_set_acceptcaps_function (trans->sinkpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_acceptcaps));
  gst_pad_set_setcaps_function (trans->sinkpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_setcaps));
  gst_pad_set_event_function (trans->sinkpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_sink_event));
  gst_pad_set_chain_function (trans->sinkpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_chain));
  gst_pad_set_activatepush_function (trans->sinkpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_sink_activate_push));
  gst_pad_set_bufferalloc_function (trans->sinkpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_buffer_alloc));
  ...
  gst_pad_set_event_function (trans->srcpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_src_event));
  gst_pad_set_checkgetrange_function (trans->srcpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_check_get_range));
  gst_pad_set_getrange_function (trans->srcpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_getrange));
  gst_pad_set_activatepull_function (trans->srcpad,
      GST_DEBUG_FUNCPTR (gst_my_base_transform_src_activate_pull));
 ...
 */
static GstCaps *
gst_my_base_transform_transform_caps (GstMyBaseTransform * trans,
    GstPadDirection direction, GstCaps * caps)
{
  ...
  klass = GST_MY_BASE_TRANSFORM_GET_CLASS (trans);
 ...
 */
static gboolean
gst_my_base_transform_transform_size (GstMyBaseTransform * trans,
    GstPadDirection direction, GstCaps * caps,
    guint size, GstCaps * othercaps, guint * othersize)
{
  ...
  if (klass->transform_size) {
    ...
    if (!gst_my_base_transform_get_unit_size (trans, caps, &inunitsize))
      goto no_in_size;
    ...
    if (!gst_my_base_transform_get_unit_size (trans, othercaps, &outunitsize))
      goto no_out_size;
 ...
 */
static GstCaps *
gst_my_base_transform_getcaps (GstPad * pad)
{
  GstMyBaseTransform *trans;
  GstPad *otherpad;
  GstCaps *caps;

  trans = GST_MY_BASE_TRANSFORM (gst_pad_get_parent (pad));
  ...
  if (caps) {
    ...
    caps = gst_my_base_transform_transform_caps (trans,
...
static gboolean
gst_my_base_transform_configure_caps (GstMyBaseTransform * trans, GstCaps * in,
    GstCaps * out)
{
  gboolean ret = TRUE;
  GstMyBaseTransformClass *klass;
  ...
  gst_my_base_transform_set_in_place (trans,
  ...
  if (klass->passthrough_on_same_caps)
    gst_my_base_transform_set_passthrough (trans, trans->have_same_caps);
 ...
 */
static gboolean
gst_my_base_transform_can_transform (GstMyBaseTransform * trans, GstPad * pad,
    GstCaps * in, GstCaps * out)
{
  ...
  othercaps =
      gst_my_base_transform_transform_caps (trans, GST_PAD_DIRECTION (pad), in);
 ...
 */
static GstCaps *
gst_my_base_transform_find_transform (GstMyBaseTransform * trans, GstPad * pad,
    GstCaps * caps)
{
  GstMyBaseTransformClass *klass;
   ...
   * passthrough at all. */
  othercaps = gst_my_base_transform_transform_caps (trans,
...
static gboolean
gst_my_base_transform_acceptcaps (GstPad * pad, GstCaps * caps)
{
  GstMyBaseTransform *trans;
#if 0
  GstPad *otherpad;
...
#endif
  {
     ...
     * transform this caps. */
    othercaps = gst_my_base_transform_find_transform (trans, pad, caps);
    if (!othercaps || gst_caps_is_empty (othercaps))
      goto no_transform_possible;
 ...
 */
static gboolean
gst_my_base_transform_setcaps (GstPad * pad, GstCaps * caps)
{
  GstMyBaseTransform *trans;
  GstPad *otherpad, *otherpeer;
  GstCaps *othercaps = NULL;
  ...
  /* call configure now */
  if (!(ret = gst_my_base_transform_configure_caps (trans, incaps, outcaps)))
...
gst_my_base_transform_prepare_output_buffer (GstMyBaseTransform * trans,
    GstBuffer * in_buf, GstBuffer ** out_buf)
{
  GstMyBaseTransformClass *bclass;
  GstMyBaseTransformPrivate *priv;
  GstFlowReturn ret = GST_FLOW_OK;
  guint outsize, newsize, expsize;
  gboolean discard, setcaps, copymeta;
  GstCaps *incaps, *oldcaps, *newcaps, *outcaps;

  bclass = GST_MY_BASE_TRANSFORM_GET_CLASS (trans);
  ...
  *out_buf = NULL;
  ...
  if (trans->passthrough) {
    ...
    if (want_in_place) {
      ...
      if (!gst_my_base_transform_transform_size (trans,
  ...
  if (newcaps && !gst_caps_is_equal (newcaps, oldcaps)) {
     ...
     * buffer later on. */
    gst_my_base_transform_transform_size (trans,
    ...
    can_convert =
	gst_my_base_transform_can_transform (trans, trans->sinkpad, incaps,
	newcaps);

    if (can_convert) {
      GST_DEBUG_OBJECT (trans, "reconfigure transform for current buffer");
      /* caps not empty, try to renegotiate to the new format */
      if (!gst_my_base_transform_configure_caps (trans, incaps, newcaps)) {
	...
	*out_buf = NULL;
      ...
      othercaps =
	  gst_my_base_transform_find_transform (trans, trans->srcpad, newcaps);
 ...
 */
static gboolean
gst_my_base_transform_get_unit_size (GstMyBaseTransform * trans, GstCaps * caps,
    guint * size)
{
  gboolean res = FALSE;
  GstMyBaseTransformClass *bclass;
 ...
 */
static GstFlowReturn
gst_my_base_transform_buffer_alloc (GstPad * pad, guint64 offset, guint size,
    GstCaps * caps, GstBuffer ** buf)
{
  ...
  *buf = NULL;
  ...
  if (same_caps) {
	...
	peercaps =
	    gst_pad_peer_get_caps_reffed (GST_MY_BASE_TRANSFORM_SINK_PAD (trans));
	...
	gst_pad_fixate_caps (GST_MY_BASE_TRANSFORM_SINK_PAD (trans), sink_suggest);
    ...
    if (sink_suggest == NULL) {
      ...
      othercaps = gst_my_base_transform_find_transform (trans, pad, sink_suggest);
      if (!othercaps || gst_caps_is_empty (othercaps)) {
...
static gboolean
gst_my_base_transform_sink_event (GstPad * pad, GstEvent * event)
{
  GstMyBaseTransform *trans;
  GstMyBaseTransformClass *bclass;
  gboolean ret = TRUE;
...
static gboolean
gst_my_base_transform_sink_eventfunc (GstMyBaseTransform * trans, GstEvent * event)
{
...
static gboolean
gst_my_base_transform_src_event (GstPad * pad, GstEvent * event)
{
  GstMyBaseTransform *trans;
  GstMyBaseTransformClass *bclass;
  gboolean ret = TRUE;
...
static gboolean
gst_my_base_transform_src_eventfunc (GstMyBaseTransform * trans, GstEvent * event)
{
  gboolean ret;
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_QOS:
    {
      ...
      gst_my_base_transform_update_qos (trans, proportion, diff, timestamp);
      break;
    }
    default:
      break;
...
static GstFlowReturn
gst_my_base_transform_handle_buffer (GstMyBaseTransform * trans, GstBuffer * inbuf,
    GstBuffer ** outbuf)
{
  GstMyBaseTransformClass *bclass;
  GstFlowReturn ret = GST_FLOW_OK;
  ...
  if (G_LIKELY ((incaps = GST_BUFFER_CAPS (inbuf)))) {
    ...
    if (G_UNLIKELY (reconfigure)) {
      ...
      if (!gst_my_base_transform_setcaps (trans->sinkpad, incaps))
	goto not_negotiated;
    }
  }
...
no_qos:
  ...
  ret = gst_my_base_transform_prepare_output_buffer (trans, inbuf, outbuf);
...
static gboolean
gst_my_base_transform_check_get_range (GstPad * pad)
{
  GstMyBaseTransform *trans;
  gboolean ret;
 ...
 */
static GstFlowReturn
gst_my_base_transform_getrange (GstPad * pad, guint64 offset,
    guint length, GstBuffer ** buffer)
{
  GstMyBaseTransform *trans;
  GstMyBaseTransformClass *klass;
  ...
  GST_MY_BASE_TRANSFORM_LOCK (trans);
  ret = gst_my_base_transform_handle_buffer (trans, inbuf, buffer);
  GST_MY_BASE_TRANSFORM_UNLOCK (trans);
...
static GstFlowReturn
gst_my_base_transform_chain (GstPad * pad, GstBuffer * buffer)
{
  GstMyBaseTransform *trans;
  GstMyBaseTransformClass *klass;
  GstFlowReturn ret;
  ...
  trans = GST_MY_BASE_TRANSFORM (GST_OBJECT_PARENT (pad));
  ...
  GST_MY_BASE_TRANSFORM_LOCK (trans);
  ret = gst_my_base_transform_handle_buffer (trans, buffer, &outbuf);
  GST_MY_BASE_TRANSFORM_UNLOCK (trans);

  /* outbuf can be NULL, this means a dropped buffer, if we have a buffer but
   * GST_MY_BASE_TRANSFORM_FLOW_DROPPED we will not push either. */
  if (outbuf != NULL) {
    if ((ret == GST_FLOW_OK)) {
...
//      gst_buffer_unref(outbuf);
  ...
  if (ret == GST_MY_BASE_TRANSFORM_FLOW_DROPPED) {
    trans->priv->discont = TRUE;
    ret = GST_FLOW_OK;
  }
...
static void
gst_my_base_transform_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstMyBaseTransform *trans;
  ...
  switch (prop_id) {
    case PROP_QOS:
      gst_my_base_transform_set_qos_enabled (trans, g_value_get_boolean (value));
      break;
    default:
...
static void
gst_my_base_transform_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstMyBaseTransform *trans;
  ...
  switch (prop_id) {
    case PROP_QOS:
      g_value_set_boolean (value, gst_my_base_transform_is_qos_enabled (trans));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static gboolean
gst_my_base_transform_activate (GstMyBaseTransform * trans, gboolean active)
{
  GstMyBaseTransformClass *bclass;
  ...
  if (active) {
    ...
    if (bclass->passthrough_on_same_caps) {
      gst_my_base_transform_set_passthrough (trans, FALSE);
    }
    gst_caps_replace (&trans->cache_caps1, NULL);
    gst_caps_replace (&trans->cache_caps2, NULL);
...
static gboolean
gst_my_base_transform_sink_activate_push (GstPad * pad, gboolean active)
{
  gboolean result = TRUE;
  GstMyBaseTransform *trans;
  ...
  result = gst_my_base_transform_activate (trans, active);
...
static gboolean
gst_my_base_transform_src_activate_pull (GstPad * pad, gboolean active)
{
...
/**
 * gst_my_base_transform_set_passthrough:
 * @trans: the #GstMyBaseTransform to set
 ...
 */
void
gst_my_base_transform_set_passthrough (GstMyBaseTransform * trans,
    gboolean passthrough)
{
  GstMyBaseTransformClass *bclass;

  g_return_if_fail (GST_IS_MY_BASE_TRANSFORM (trans));
...
/**
 * gst_my_base_transform_is_passthrough:
 * @trans: the #GstMyBaseTransform to query
 *
 ...
 */
gboolean
gst_my_base_transform_is_passthrough (GstMyBaseTransform * trans)
{
  gboolean result;

  g_return_val_if_fail (GST_IS_MY_BASE_TRANSFORM (trans), FALSE);
...
/**
 * gst_my_base_transform_set_in_place:
 * @trans: the #GstMyBaseTransform to modify
 ...
 */
void
gst_my_base_transform_set_in_place (GstMyBaseTransform * trans, gboolean in_place)
{
  GstMyBaseTransformClass *bclass;
...
/**
 * gst_my_base_transform_is_in_place:
 * @trans: the #GstMyBaseTransform to query
 *
 ...
 */
gboolean
gst_my_base_transform_is_in_place (GstMyBaseTransform * trans)
{
  gboolean result;
...
/**
 * gst_my_base_transform_update_qos:
 * @trans: a #GstMyBaseTransform
 ...
 */
void
gst_my_base_transform_update_qos (GstMyBaseTransform * trans,
...
/**
 * gst_my_base_transform_set_qos_enabled:
 * @trans: a #GstMyBaseTransform
 * @enabled: new state
 *
 ...
 */
void
gst_my_base_transform_set_qos_enabled (GstMyBaseTransform * trans, gboolean enabled)
{
  g_return_if_fail (GST_IS_MY_BASE_TRANSFORM (trans));
...
/**
 * gst_my_base_transform_is_qos_enabled:
 * @trans: a #GstMyBaseTransform
 *
 ...
 */
gboolean
gst_my_base_transform_is_qos_enabled (GstMyBaseTransform * trans)
{
  gboolean result;
...
/**
 * gst_my_base_transform_set_gap_aware:
 * @trans: a #GstMyBaseTransform
 * @gap_aware: New state
 *
 ...
 */
void
gst_my_base_transform_set_gap_aware (GstMyBaseTransform * trans, gboolean gap_aware)
{
  g_return_if_fail (GST_IS_MY_BASE_TRANSFORM (trans));
 ...
 */
void
gst_my_base_transform_suggest (GstMyBaseTransform * trans, GstCaps * caps,
    guint size)
{
  g_return_if_fail (GST_IS_MY_BASE_TRANSFORM (trans));
...
/**
 * gst_my_base_transform_reconfigure:
 * @trans: a #GstMyBaseTransform
 *
 ...
 */
void
gst_my_base_transform_reconfigure (GstMyBaseTransform * trans)
{
  g_return_if_fail (GST_IS_MY_BASE_TRANSFORM (trans));
