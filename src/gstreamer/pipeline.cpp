...
//	d->subOverlay = new TextOverlay;
	...
	gst_bin_add_many(GST_BIN(d->videoBin), vqueue, vconv, vrate
//			, d->videoBox, d->infoOverlay->element(), d->subOverlay->element()
			, d->videoSink, NULL);
	gst_element_link_many(vqueue, vconv, vrate
//			, d->videoBox, d->infoOverlay->element(), d->subOverlay->element()
			, d->videoSink, NULL);
...
void Pipeline::cbNotifyVideoCaps(GObject *obj, GParamSpec *pspec, gpointer data) {
	...
	if ((str = gst_caps_get_structure(caps, 0))) {
		if (gst_structure_get_int(str, "width", &width)
			...
			emit self->videoSizeChanged(QSize(width, height));
		}
	}
	gst_caps_unref(caps);
}
...
void Pipeline::setWidget(QWidget *widget) {
	reinterpret_cast<QWidgetVideoSink*>(d->videoSink)->sink = widget;
}
